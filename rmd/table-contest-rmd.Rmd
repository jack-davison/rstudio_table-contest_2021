...
openair_image <- function(plot_object, height) {
  ...
  invisible(vapply(seq_along(plot_object),
...
data_plots <- data |>
  nest_by(species) |>
  mutate(
    polrose = list(pollutionRose(data, pollutant = "conc")$plot),
...
function(plot_object, height = 100, aspect_ratio = 1) {
  ...
  vapply(seq_along(plot_object),
    FUN.VALUE = character(1),
    USE.NAMES = FALSE, FUN = function(x) {
      filename <- paste0("temp_ggplot_", formatC(x,
        width = 4,
        flag = "0"
      ), ".png")
      ggplot2::ggsave(
        filename = filename, plot = plot_object[[x]],
